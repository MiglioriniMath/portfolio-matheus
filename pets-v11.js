(() => {
  ...
  function seatPets() {
    ...
    const baseY = clamp(footerTop - (isMobile ? 13 : 12), 95, window.innerHeight - 42);
    const right = window.innerWidth - (isMobile ? 27 : 34);
  ...
  function wakeEarly(e) {
    ...
    const now=performance.now();
    if (lastTarget===e.currentTarget && now-lastTap<420) {
      clearTimeout(startTimer); activate(); lastTap=0; lastTarget=null;
    } else { lastTap=now; lastTarget=e.currentTarget; }
  }
  ...
  seatPets();
  if (!reducedMotion) startTimer=window.setTimeout(activate,AUTO_START_MS);
})();
